          ...
          <tbody>
            <template v-if="Array.isArray(tableData) && tableData.length > 0">
              <tr v-for="(item, index) in sortedData" :key="index">
                ...
                <td>{{ item.end_time }}</td>
                <td>
                  <button type="button" class="btn btn-outline-primary btn-sm me-3" @click="openModal(item.id)">
                    <i class="fas fa-edit" title="edit entry"></i>
                  </button>
                  <button type="button" class="btn btn-outline-danger btn-sm" @click="confirmCancel(item.id)">
