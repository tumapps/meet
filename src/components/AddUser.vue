...
<template>
    <b-modal v-model="showModal" no-close-on-backdrop id="addUserModal" title="Add User" hide-footer size="xl" centered body-class="p-4 custom-modal-body" >
        <b-container class="p-3"> <!-- Container with padding -->
            <h3 class="mb-4">Account Information:</h3>
            <form @submit.prevent="submitForm">
                <b-form-group label="Email: *" label-class="mb-2">
                    <b-form-input type="email" placeholder="Email address" v-model="formData.email_address" required
                        class="mb-3" />
                    <div v-if="errors.email_address" class="errors">{{ errors.email_address }}</div>
                </b-form-group>

                <b-form-group label="Username: *" label-class="mb-2">
                    <b-form-input type="text" placeholder="Username" v-model="formData.username" required
                        class="mb-3" />
                    <div v-if="errors.username" class="errors">{{ errors.username }}</div>
                </b-form-group>

                <b-form-group label="Password: *" label-class="mb-2">
                    <b-form-input type="password" placeholder="Password" v-model="formData.password" required
                        class="mb-3" />
                    <div v-if="errors.password" class="errors">{{ errors.password }}</div>
                </b-form-group>

                <b-form-group label="Confirm Password: *" label-class="mb-2">
                    <b-form-input type="password" placeholder="Confirm Password" v-model="formData.confirm_password"
                        required class="mb-4" />
                ...
                <b-form-group label="First Name: *" label-class="mb-2">
                    <b-form-input type="text" placeholder="First Name" v-model="formData.first_name" required
                        class="mb-3" />
                ...
                <b-form-group label="Last Name: *" label-class="mb-2">
                    <b-form-input type="text" placeholder="Last Name" v-model="formData.last_name" required
                        class="mb-3" />
                    <div v-if="errors.last_name" class="errors">{{ errors.last_name }}</div>
                </b-form-group>

                <b-form-group label="Contact Number: *" label-class="mb-2">
                    <b-form-input type="text" placeholder="Contact Number" v-model="formData.mobile_number" required
                        class="mb-4" />
                    <div v-if="errors.mobile_number" class="errors">{{ errors.mobile_number }}</div>
                </b-form-group>

                <div class="text-end mt-4">
                    <button v-if="!submitted" type="submit" class="btn btn-primary">
                        Submit
                    </button>
